...
def respond_from_two_models(
    ...
    stream_2 = client.chat.completions.create(
        model="gpt-4o-mini-2024-07-18",  # Example for another model
        messages=messages,
        max_tokens=max_tokens,
        stream=True,
        temperature=temperature,
...
# Function to capture feedback and correct answer
def feedback_good(feedback_text, correct_answer):
    feedback = {
        "rating": "GOOD",
    ...
    save_conversation()
    return f"Feedback: The answer was marked as GOOD. Correct Answer: {correct_answer}. Additional feedback: {feedback_text}"


def feedback_not_good(feedback_text, correct_answer):
    feedback = {
        "rating": "NOT GOOD",
        "feedback_text": feedback_text,
        "correct_answer": correct_answer
    ...
    conversation_log.append({"feedback": feedback})
    save_conversation()
    return f"Feedback: The answer was marked as NOT GOOD. Correct Answer: {correct_answer}. Additional feedback: {feedback_text}"
...
# Function to clear the history and reset the UI
def clear_history():
    global conversation_log
    conversation_log = []  # Clear the conversation log
    return "", "", "", "", ""  # Clear user input, responses, feedback, and correct answer
...
with gr.Blocks() as demo:
    ...
    with gr.Column(elem_id="col_container", visible=False) as main_block:
        ...
        correct_answer_input = gr.Textbox(placeholder="Input correct answer (optional)", label="Correct Answer")
        ...
        # Handle feedback submission with correct answer and additional feedback
        good_feedback_button.click(feedback_good, inputs=[feedback_text, correct_answer_input], outputs=feedback_output)
        not_good_feedback_button.click(feedback_not_good, inputs=[feedback_text, correct_answer_input],
                                       outputs=feedback_output)

        # Clear button to reset conversation and UI
        clear_button = gr.Button("Clear History")
        clear_button.click(clear_history,
                           outputs=[user_input, model_1_output, model_2_output, feedback_text, correct_answer_input])
